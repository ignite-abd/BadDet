...
class Classification(nn.Module):
    ...
    def forward(self, embeds):
        ...
        x = F.elu_(self.fc2(x))
        logists = torch.log_softmax(x, 1)    # dim=1：对每一行的所有元素进行softmax运算，并使得每一行所有元素和为1。
        return logists
...
class UnsupervisedLoss(object):
    ...
    def get_loss_unsup(self, embeddings, nodes):
        ...
        #print(embeddings)   #[0.0000, 0.0554, 0.0966,  ..., 0.0000, 0.0000, 0.0582]
        #print(nodes)   #[   0    1    2 ... 3272 3273 3274]
        #print(len(embeddings))
        #print(len(self.unique_nodes_batch))
        assert False not in [nodes[i]==self.unique_nodes_batch[i] for i in range(len(nodes))]
        node2index = {n:i for i,n in enumerate(self.unique_nodes_batch)}
        #print(node2index)  #{0: 0, 1: 1, ..., 3265: 1814, 3267: 1815, 3268: 1816, 3272: 1817, 3273: 1818, 3274: 1819}
        #print("node2index:" + str(node2index))
        nodes_score_mean = []
        nodes_score_maxmin = []
        assert len(self.node_positive_pairs) == len(self.node_negative_pairs)
        #print(len(self.node_negative_pairs))
...
def data_loader(data_dir):
    feature_pd = pd.read_csv("{}BTC_node_attributes_new.txt".format(data_dir), header=None, delimiter=",")
    # feature_pd = pd.read_csv("{}BTC_node_attributes_new_new_new.txt".format(data_dir), header=None, delimiter=",")
    feature = feature_pd.to_numpy()  # [:, 53:]
    print("Feature shape:", feature.shape)
    print(type(feature))

    label_pd = pd.read_csv("{}BTC_node_labels_new.txt".format(data_dir), header=None)
    label = label_pd.to_numpy().squeeze()
    print("Label shape:", label.shape)

    edge_pd = pd.read_csv("{}BTC_A_new.txt".format(data_dir), header=None)
    edge = edge_pd.to_numpy()
    # edge = edge - 1
    print("Edge shape:", edge.shape)

    label, classes_dict = encode_onehot(label)

    new_classes_dict = {str(np.argmax(value)): key for key, value in classes_dict.items()}
    reversed_dict = {value: key for key, value in new_classes_dict.items()}
    print('-' * 100)
    print(classes_dict)
    print('-' * 100)
    ...
    mask = (label != int(reversed_dict['Ts_node'])) & (label != int(reversed_dict['5']))
    train_idx_tensor = torch.nonzero(mask).squeeze()
    train_idx = train_idx_tensor.numpy()




    train_idx, val_idx = train_test_split(train_idx, test_size=0.2, random_state=42)
    train_idx, test_idx = train_test_split(train_idx, test_size=0.25, random_state=42)
    ...
    return data, new_classes_dict

class CustomGCNLayer(MessagePassing):
    def __init__(self, in_channels, out_channels):
        super(CustomGCNLayer, self).__init__(aggr='add')  # "Add" aggregation.
        self.lin = torch.nn.Linear(in_channels, out_channels)
        self.weight = self.lin.weight  # Now you can access the weight attribute
...
class GCN(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels):
        ...
        self.conv1 = CustomGCNLayer(in_channels, hidden_channels)
        self.conv2 = CustomGCNLayer(hidden_channels, out_channels)
...
def train(model, optimizer, data):
    ...
    weight_matrix1 = model.conv1.weight.data
    weight_matrix2 = model.conv2.weight.data

    return loss.item(), accuracy, weight_matrix1, weight_matrix2


def result_figure(true_labels, predicted_labels):
    ...
    plt.bar(x, f1, width=0.2, label='F1', align='edge')
...
def test(model, data, new_classes_dict, name_dict):
    model.eval()
    ...
    for _, mask in data('test_mask'):
        ...
        precision, recall, f1, _ = precision_recall_fscore_support(true_labels, predicted_labels, labels=unique_labels,
                                                                   average=None)

        for label, p, r, f in zip(unique_labels, precision, recall, f1):
            print(f'Label: {name_dict[str(new_classes_dict[str(label)])]}, Precision: {p}, Recall: {r}, F1: {f}')


        precision, recall, f1, _ = precision_recall_fscore_support(true_labels, predicted_labels, average='weighted')
        print(f"Total result: Precision: {precision}, Recall: {recall}, F1: {f1}")

    return accs

def baddet(data_dir, n_epoch):
    data, new_classes_dict = data_loader(data_dir)
    model = GCN(data.x.shape[1], 32, data.y.max().item() + 1)
    ...
    load_weight = True
    if load_weight == True:
        file_name_1 = 'weight_matrix1.pkl'
        file_name_2 = 'weight_matrix2.pkl'

        with open(file_name_1, 'rb') as file_1:
            weight_matrix1_new = pickle.load(file_1)

        with open(file_name_2, 'rb') as file_2:
            weight_matrix2_new = pickle.load(file_2)

        with torch.no_grad():
            model.conv1.weight.data = weight_matrix1_new.squeeze(0)
            model.conv2.weight.data = weight_matrix2_new.squeeze(0)

    for epoch in range(n_epoch):
        train_loss, train_accuracy, weight_matrix1, weight_matrix2 = train(model, optimizer, data)
    ...
    test_accs = test(model, data, new_classes_dict, name_dict)
    print("Test Accuracy: {:.4f}".format(test_accs[-1]))
    return weight_matrix1, weight_matrix2
...
class GNN(nn.Module):
    ...
    def forward(self, nodes_batch):
        ...
        #print("here:" + str(lower_layer_nodes))
