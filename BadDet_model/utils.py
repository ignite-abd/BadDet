...
def train_classification(Dc, gnn, classification, ds, device, max_vali_f1, outer_epoch, epochs=500):
    ...
    # '--over_sample', type=str, default='none'
...
def train_model(Dc, args, gnn, classification, unsupervised_loss, device, outer_epoch):
    ...
    labels = getattr(Dc, ds+'_labels')
    #print("train_nodes:" + str(train_nodes))
    ...
    batches = math.ceil(len(train_nodes) / args.b_sz)           #  '--b_sz', type=int, default=100

    visited_nodes = set()
    #print("visited_nodes:" + str(visited_nodes))
    for index in range(batches):
        ...
        #print("nodes_batch_1:" + str(nodes_batch))
        nodes_batch = np.asarray(list(unsupervised_loss.extend_nodes(nodes_batch, num_neg=50)))
        #print("nodes_batch_2:" + str(len(nodes_batch)))
        visited_nodes |= (set(nodes_batch) & set(train_nodes))
        labels_batch = labels[nodes_batch]
        #print("nodes_batch:" + str(nodes_batch))
        embs_batch = gnn(nodes_batch)
        #print("embs_batch:" + str(embs_batch.shape))
        #print("nodes_batch:" + str(len(nodes_batch)))
        ...
        # '--loss', type=str, default='1010'
        # '--a_loss', type=str, default='none'
        #print(args.loss[0])
        #print(args.a_loss[0])
        if int(args.loss[0]):  # 1
            loss += loss_maxmin
        if int(args.loss[1]):
            loss += loss_mean
        if args.a_loss != 'none':  # n
